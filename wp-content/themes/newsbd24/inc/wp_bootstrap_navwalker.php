...
class newsbd24_bootstrap_navwalker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		$output .= "\n$indent<ul  class=\"dropdown-menu\" aria-labelledby=\"dropdown02\">\n";
	}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			if ( $args->has_children &&  $depth == 0 ){
				$class_names .= ' nav-item dropdown has-submenu';
			...
			$item_output .= '<a'. $attributes .' role="menu">';
			...
			if ( $args->has_children  ) {
				//$item_output .= '<i class="fa fa-angle-down newsbd24__drop__toggle"></i>';
			}
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">'.esc_html__( 'Add a menu','newsbd24') .'</a></li>';
			$fb_output .= '</ul>';
