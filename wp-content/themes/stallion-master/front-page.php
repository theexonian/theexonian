...
<?php while ($queryExposition->have_posts()) : $queryExposition->the_post(); ?>
					<div class="breaking-mainelement">
						<div class="breaking-photo" style="background:url(<?= bw_FeaturedImage($post->ID,"header-wide",bw_CheckPhotoAlign(get_field('photo_alignment'),'c')); ?>)">
...
if ($featureDisplay == "horizontal" && has_post_thumbnail()) : ?>
					<div class="exposition-mainelement exposition-horizontal-mainelement">
						<div class="exposition-photo" style="background-image:url(<?php echo bw_FeaturedImage($post->ID,"header-medium",bw_CheckPhotoAlign(get_field('photo_alignment'),'c')); ?>)">
...
<?php elseif ($featureDisplay == "vertical" && has_post_thumbnail()) : ?>
					<div class="exposition-mainelement exposition-vertical-mainelement">
						<div class="exposition-photo" style="background:url(<?php echo bw_FeaturedImage($post->ID,"medium-vertical",bw_CheckPhotoAlign(get_field('photo_alignment'))); ?>)">
							<a href="<?php the_permalink(); ?>" class="exposition-photo-link">
							</a>
...
?>
								<div class="matrixexpo matrixexpo-<?php echo $matrixexpoPosition; ?> <?php echo (has_post_thumbnail()) ? "matrixexpo-hasphoto" : "matrixexpo-bg" . rand(1,5); ?>" <?php if(has_post_thumbnail()) : ?>style="background:url(<?php echo bw_FeaturedImage($post->ID,"header-small",bw_CheckPhotoAlign(get_field('photo_alignment'))); ?>); background-size:190px"<?php endif; ?>>
...
<?php while ($queryLowerexpo->have_posts()) : $queryLowerexpo->the_post(); ?>
							<div class="lowerexpo <?php echo (has_post_thumbnail()) ? "lowerexpo-hasphoto" : "lowerexpo-nophoto"; ?>">
								<div class="lowerexpo-photo" <?php if(has_post_thumbnail()) : ?>style="background:url(<?php echo bw_FeaturedImage($post->ID,"header-small",bw_CheckPhotoAlign(get_field('photo_alignment'),'c')); ?>)"<?php endif; ?>>
...
<?php if (has_post_thumbnail()) { ?>
							<li class="news-ticker-containsimage">
								<a href="<?php the_permalink(); ?>"><img class="news-ticker-image pull-right" src="<?php echo bw_FeaturedImage($post->ID,"small-flexwidth",bw_CheckPhotoAlign(get_field('photo_alignment'))); ?>"></a>
