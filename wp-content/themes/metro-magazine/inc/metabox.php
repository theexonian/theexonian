...
function metro_magazine_save_sidebar_layout( $post_id ){
    global $metro_magazine_sidebar_layout , $post;
    ...
    foreach( $metro_magazine_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, 'metro_magazine_sidebar_layout', true ); 
        $new = sanitize_text_field( $_POST['metro_magazine_sidebar_layout'] );
        if ( $new && $new != $old ) {  
            update_post_meta($post_id, 'metro_magazine_sidebar_layout', $new );  
        } elseif ('' == $new && $old ) {  
            delete_post_meta( $post_id,'metro_magazine_sidebar_layout',  $old );  
        } 
     } // end foreach     
}
