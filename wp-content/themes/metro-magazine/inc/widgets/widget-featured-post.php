...
// register Metro_Magazine_Featured_Post widget
function metro_magazine_register_featured_post_widget() {
    register_widget( 'Metro_Magazine_Featured_Post' );
}
add_action( 'widgets_init', 'metro_magazine_register_featured_post_widget' );
 
 /**
 * Adds Metro_Magazine_Featured_Post widget.
 */
class Metro_Magazine_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'metro_magazine_featured_post', // Base ID
			__( 'RARA: Featured Post', 'metro-magazine' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'metro-magazine' ), ) // Args
		);
	}
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'metro-magazine-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
		$postlist[0] = array(
    		'value' => 0,
    		'label' => __('--Choose--', 'metro-magazine'),
    	);
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'metro-magazine' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'metro-magazine' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'metro-magazine' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'metro-magazine' ); ?></label>
		</p>
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'metro-magazine' );
...
} // class Metro_Magazine_Featured_Post