...
// register Metro_Magazine_Recent_Post widget
function metro_magazine_register_recent_post_widget() {
    register_widget( 'Metro_Magazine_Recent_Post' );
}
add_action( 'widgets_init', 'metro_magazine_register_recent_post_widget' );
 
 /**
 * Adds Metro_Magazine_Recent_Post widget.
 */
class Metro_Magazine_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'metro_magazine_recent_post', // Base ID
            __( 'RARA: Recent Post', 'metro-magazine' ), // Name
            array( 'description' => __( 'A Recent Post Widget', 'metro-magazine' ), ) // Args
        );
    }
                    ...
                    <li>
                        <?php if( has_post_thumbnail() && $show_thumb ){ ?>
                            <a href="<?php the_permalink();?>" class="post-thumbnail">
                                <?php the_post_thumbnail( 'metro-magazine-recent-post' );?>
                            </a>
                        <?php }?>
                            ...
                            <?php if( $show_date ) { ?>
                                <div class="entry-meta">
                                    <span class="posted-on"><a href="<?php the_permalink(); ?>">
                                        <time datetime="<?php printf( __( '%1$s', 'metro-magazine' ), get_the_date('Y-m-d') ); ?>"><?php printf( __( '%1$s', 'metro-magazine' ), get_the_date('F j, Y') ); ?></time></a>
                                    </span>
                                </div>
                            <?php } ?>
                        </div>                        
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'metro-magazine' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'metro-magazine' ); ?></label>
        </p>
        <?php 
    }
...
} // class Metro_Magazine_Recent_Post 