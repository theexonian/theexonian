...
<script type="text/javascript" src="https://stuartrucker.github.io/samsidea/google-spreadsheet.js"></script>
<script type="text/javascript" src="https://stuartrucker.github.io/samsidea/tabletop.js"></script>
<script type="text/javascript" src="https://stuartrucker.github.io/samsidea/henryscript.js"></script>
...
?>" />
<title><?php
echo apply_filters('password_protected_wp_title', get_bloginfo('name'));
?></title>
...
<div id="login" style = "width: 500px">
    <h1><a href="<?php
echo esc_url(apply_filters('password_protected_login_headerurl', home_url('/')));
?>" title="<?php
echo esc_attr(apply_filters('password_protected_login_headertitle', get_bloginfo('name')));
?>"><?php
bloginfo('name');
?></a></h1>

    <?php
do_action('password_protected_login_messages');
?>
   <?php
do_action('password_protected_before_login_form');
?>

    <form name="loginform" id="loginform" action="<?php
echo esc_url($Password_Protected->login_url());
?>" method="post">
...
_e('Password', 'password-protected');
?> <span style = "float: right; "><small>Hint: "<a onclick = "autoFill()">FreePress</a>"</small></span>
        ...
        <!--
        <p class="forgetmenot"><label for="rememberme"><input name="rememberme" type="checkbox" id="rememberme" value="forever" tabindex="90"<?php
checked(!empty($_POST['rememberme']));
?> /> <?php
esc_attr_e('Remember Me', 'password-protected');
?></label></p>
        -->
        <p class="submit">
            <input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php
esc_attr_e('Log in', 'password-protected');
?>" tabindex="100" />
            <input type="hidden" name="testcookie" value="1" />
            <input type="hidden" name="password-protected" value="login" />
            <input type="hidden" name="redirect_to" value="<?php
echo esc_attr($_REQUEST['redirect_to']);
?>" />
        </p>
    ...
    <?php
do_action('password_protected_after_login_form');
?>
...
function setCookie(cname, cvalue, exdays) {
  ...
  $("#loginform").submit(function(){
    setCookie("email_for_subscription", $("#email-logger").val(), 3);
    setCookie("message_for_subscription", confirmemail($("#email-logger").val()), 3);
    if($("#loginform").valid()){
