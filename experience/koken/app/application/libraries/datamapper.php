...
class DataMapper implements IteratorAggregate {
	...
	public function save($object = '', $related_field = '')
	{
		...
		if ($this->valid)
		{
			...
			{
				// HACK: We need both to be saved here, plus captured on if empty
				$this->{$this->created_field} = $this->{$this->updated_field} = $timestamp;
				if (in_array('captured_on', $this->fields) && empty($this->captured_on))
				{
					$this->captured_on = $timestamp;	
				}
				// END HACK
			}

			// HACK
			if (in_array('published_on', $this->fields) && empty($this->published_on) && $this->published)
			{
				$this->published_on = $timestamp;
			}
			// /HACK

			// HACK
			if (in_array('favorited_on', $this->fields))
			{
				if ($this->favorite && empty($this->favorited_on))
				{
					$this->favorited_on = $timestamp;	
				} else if (!$this->favorite)
				{
					$this->favorited_on = null;
				}
			}

			if (in_array('featured_on', $this->fields) && isset($this->featured))
			{
				if ($this->featured && empty($this->featured_on))
				{
					$this->featured_on = $timestamp;	
				} 
				else if (!$this->featured)
				{
					$this->featured_on = null;
				}
			}
			// /HACK
