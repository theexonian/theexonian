...
class Album extends DataMapper {
	...
	function listing($params)
	{
		...
		if (isset($params['order_by']) && !isset($params['order_direction']) && in_array($params['order_by'], array('created_on', 'modified_on', 'total_count', 'image_count', 'video_count')))
		{
		...
		else if (!$sub_list && !$options['tags'] && !$options['year'] && !$options['flat'])
		{
			$this->where('level', 1);
		}
	...
	function to_array($options = array())
	{
		...
		if ($data['level'] > 1 && (!array_key_exists('include_parent', $options) || $options['include_parent']))
		{
			...
			$data['parent'] = $parent->to_array(array('with_topics' => $options['with_topics']));
		}
