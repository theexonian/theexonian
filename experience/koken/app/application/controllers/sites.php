		...
		foreach($files as $file)
		{
			...
			if (isset($info['extension']) && $info['extension'] === 'lens' && $info['filename'] !== 'error' && !isset($template_info['templates'][$info['filename']]))
			{
				$template_info['templates'][$info['filename']] = array(
					'name' => ucwords( preg_replace('/[^a-z0-9]/', ' ', strtolower($info['filename'])) )
				);
			}
		}
			...
			function build_autos($items, $data, $user)
			{
				foreach($items as $index => &$item)
				{
					if (isset($item['auto']))
					{
						...
						else if (preg_match('/s$/', $item['auto']))
						{
							if ($item['auto'] === 'archives' && isset($item['year']))
							{
								$item['path'] = str_replace(':year', $item['year'], $data['urls']['archives']);
								$item['path'] = str_replace(':month', is_numeric($item['month']) ? $item['month'] : '', $item['path']);
							}

							if (strpos($item['auto'], '_') !== false) {
								foreach(array('id', 'slug', 'month', 'year') as $id)
								{
									if ($id === 'month' && isset($item['month']) && $item['month'] === 'any')
									{
										$item['month'] = '';
									}
							...
							{
								$item['label'] = $data['url_data'][rtrim($item['auto'], 's')]['plural'];
							}
						}
						else
						{
			...
			switch($this->method)
			{
				case 'put':
					...
					else
					{
						...
						{
							$source = rtrim($data['url_data_send']['source'], 's');
							$u = new Url;
